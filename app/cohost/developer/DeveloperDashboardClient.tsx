...
export default function DeveloperDashboardClient({ adminEmail }: { adminEmail: string }) {
    ...
    // Hosts tab derived state
    const hostsSubTabs: { id: HostSubTab; label: string; count: number; color: string }[] = [
    ...
    const hostsDisplayUsers = hostSubTab === 'active' ? activeHosts
        : hostSubTab === 'issues' ? issueHosts
        : inactiveHosts;
    const hostsFilteredUsers = showNeedsAttentionOnly
        ? hostsDisplayUsers.filter(u => needsAttentionHosts.includes(u))
        : hostsDisplayUsers;

    // Health tab derived state
    const healthBroken = users.filter(u => u.gmail_broken);
    const healthNoConnections = users.filter(u => u.workspace_id && !u.gmail_connected && !u.has_ical && u.property_count > 0);
                    ...
                    <>
                                ...
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    {showNeedsAttentionOnly && (
                                        <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full font-medium">
                                            {hostsFilteredUsers.length} need attention
                                        </span>
                                    )}
                                </div>
                        ...
                        {activeTab === 'health' && (
                            <div className="space-y-6">
                                {healthBroken.length > 0 && (
                                    <div>
                                        <h3 className="text-sm font-semibold text-red-700 mb-2">⚠ Broken Gmail Connections ({healthBroken.length})</h3>
                                ...
                                {healthNoConnections.length > 0 && (
                                    <div>
                                        <h3 className="text-sm font-semibold text-yellow-700 mb-2">Hosts With Properties But No Connections ({healthNoConnections.length})</h3>
                                ...
                                {healthBroken.length === 0 && healthNoConnections.length === 0 && (
