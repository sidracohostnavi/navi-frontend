...
export default function CalendarSettingsPage() {
    ...
    const [syncing, setSyncing] = useState(false); // New state // OLD state name kept for compatibility but simplified usage
    const [syncingFeedId, setSyncingFeedId] = useState<string | null>(null); // Track individual feed sync
    const [viewingFeedId, setViewingFeedId] = useState<string | null>(null); // For modal
    const [isAdding, setIsAdding] = useState(false);

    // Form State
    const [newFeedName, setNewFeedName] = useState('');
    const [selectedFeedType, setSelectedFeedType] = useState(''); // Separate dropdown state
    ...
    const handleAddFeed = async () => {
        // Use custom name if 'Other' is selected, otherwise use the dropdown selection
    ...
    const handleSyncFeed = async (feedId: string) => {
        ...
        try {
            ...
            if (res.ok) {
                // Refresh just this feed's status in the list
    ...
    const handleSyncNow = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Refresh feeds to show updated sync timestamps
    ...
    if (properties.length === 0) {
        return (
            <div className="flex bg-gray-50 min-h-[calc(100vh-64px)] items-center justify-center">
    ...
    return (
        <div className="flex bg-gray-50 min-h-[calc(100vh-64px)]">
            ...
            <aside className="w-80 bg-white border-r border-gray-200 flex flex-col fixed-height overflow-y-auto">
                {/* ... (unchanged) ... */}
                <div className="p-6 border-b border-gray-100">
                    <h2 className="text-lg font-semibold text-gray-900">Properties</h2>
                    <p className="text-sm text-gray-500 mt-1">Select a property to manage sync settings.</p>
                </div>
                <div className="flex-1 overflow-y-auto w-full">
                    {properties.map(property => (
                        <button
                            ...
                            className={`w-full text-left px-6 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors flex flex-col gap-1
                ${selectedPropertyId === property.id ? 'bg-blue-50/50 border-r-4 border-r-blue-500' : 'border-r-4 border-r-transparent'}
              `}
                        >
                            <span className={`font-medium ${selectedPropertyId === property.id ? 'text-blue-700' : 'text-gray-900'}`}>
                                {property.name}
                            </span>
                            {property.address && (
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                        ...
                        <header className="mb-8 flex items-center justify-between">
                            ...
                            <div>
                                ...
                                <p className="text-gray-500">{selectedProperty.name} </p>
                            </div>
                            <button
                                onClick={handleSyncNow}
                                disabled={syncing || feeds.length === 0}
                                className="px-4 py-2 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2 shadow-sm"
                            >
                                {syncing ? (
                                    <>
                        ...
                        {/* Outbound Sync Section (unchanged) */}
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        // If not 'Other', update the name immediately
                                                        if (val !== 'Other') {
                                                            setNewFeedName(val);
                                                        } else {
                                                            setNewFeedName(''); // Clear for custom input
                                                        }
                                                    }}
    ...
    useEffect(() => {
        async function fetchBookings() {
            const { data } = await supabase
                ...
                .limit(50); // Limit to 50 for quick check
