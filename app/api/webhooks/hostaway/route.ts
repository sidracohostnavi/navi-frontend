// /app/api/webhooks/hostaway/route.ts
// Webhook endpoint for Hostaway message events
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract message data from Hostaway payload
    // Hostaway sends: conversationId, reservationId, listingMapId, body, isIncoming, etc.
    const conversationId = payload.conversationId || payload.conversation_id
    const isIncoming = payload.isIncoming === 1 || payload.isIncoming === true
    ...
    const externalConversationId = String(conversationId)
    const externalMessageId = payload.id ? String(payload.id) : null
    const messageBody = payload.body || ''
    const guestName = payload.guestName || payload.senderName || null
    const propertyId = payload.listingMapId ? String(payload.listingMapId) : null
