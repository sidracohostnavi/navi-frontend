...
async function run() {
    try {
        ...
        const formattedFeeds = feeds.map(f => ({
            property: propsMap[f.property_id]?.replace(' Estate', '').replace(' Cottage', '') || f.property_id,
            source: f.source_name.trim(),
            last_event_count: f.last_event_count
        }));
        console.table(formattedFeeds);

        console.log(`\n=== 2. Feed count vs Database count Summary ===`);
        console.log(`Property`.padEnd(20, ' ') + ` | ` + `Feeds`.padEnd(35, ' ') + ` | ` + `Total Raw Events`.padEnd(18, ' ') + ` | ` + `Bookings in DB (Mar 1+)`);
        console.log('-'.repeat(105));

        const propertyToEventsMap = feeds.reduce((acc, f) => {
            if (!acc[f.property_id]) acc[f.property_id] = { sources: [], eventSum: 0 };
            acc[f.property_id].sources.push(f.source_name.trim());
        ...
        allPropIds.forEach(propId => {
            if (propId === '22222222-2222-4222-8222-222222222222') return;
            const propName = propsMap[propId]?.replace(' Estate', '').replace(' Cottage', '') || 'Unknown';
            ...
            console.log(`${propName.padEnd(20, ' ')} | ${sourceStr.padEnd(35, ' ')} | ${String(eventSumInFeeds).padEnd(18, ' ')} | ${bookingsInDb}`);
        });
